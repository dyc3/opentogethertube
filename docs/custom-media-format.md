...
* `contentType`: A string representing the MIME type of the track. Supported values are:
  * `text/vtt` (WebVTT)
  * `text/x-ssa` (SubStation Alpha/Advanced SubStation Alpha, i.e. `.ssa`/`.ass`)
...
Browsers block requests for text tracks hosted on different domains than the current page by default. To make text tracks work cross-origin, your host needs to serve the VTT/ASS file with the `Access-Control-Allow-Origin` HTTP header.
...
```json
{
  ...
  "textTracks": [
    ...
    {
      "url": "https://example.com/subtitles_en.ass",
