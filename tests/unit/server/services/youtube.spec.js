...
jest.spyOn(redisClient, 'get').mockImplementation((key, callback) => callback(null, null));
...
describe("fetchVideoInfo", () => {
  ...
  apiGet.mockReturnValue(Promise.resolve({
...
describe("resolveURL", () => {
  const adapter = new YouTubeAdapter("");
  const apiGet = jest.fn().mockReturnValue(Promise.resolve({
    data: { items: [] },
  }));
  adapter.api.get = apiGet;
