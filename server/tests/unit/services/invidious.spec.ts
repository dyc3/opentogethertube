...
describe("fetchVideoInfo: chooses HLS/DASH by top bitrate", () => {
    ...
    function mockVideoJson() {
        (adapter.api.get as any) = vi.fn()
            // first call: /api/v1/videos?id&local=1
            .mockResolvedValueOnce({ data: { title: "T", lengthSeconds: 10, formatStreams: [] }, headers: {} })
    ...
    it("picks DASH when DASH topKbps > HLS", async () => {
        ...
        vi.spyOn(adapter as any, "probeManifest").mockImplementation(async (...args: any[]) => {
            ...
            return kind === "dash"
                ? { kind: "dash", url: "https://inv.nadeko.net/api/manifest/dash/id/abc?local=1&source=youtube", topKbps: 5000 }
                : { kind: "hls", url: "https://inv.nadeko.net/api/manifest/hls/id/abc?local=1&source=youtube", topKbps: 3000 };
        });
    ...
    it("picks HLS when HLS topKbps > DASH", async () => {
        ...
        vi.spyOn(adapter as any, "probeManifest").mockImplementation(async (...args: any[]) => {
            const kind = args[2] as "dash" | "hls";
            return kind === "dash"
                ? { kind: "dash", url: "https://inv.nadeko.net/api/manifest/dash/id/abc?local=1&source=youtube", topKbps: 2000 }
                : { kind: "hls", url: "https://inv.nadeko.net/api/manifest/hls/id/abc?local=1&source=youtube", topKbps: 4000 };
        });
    ...
    it("prefers best MP4 by qualityLabel and falls back to mp4 MIME via itag", async () => {
        ...
        (adapter.api.get as any) = vi.fn().mockResolvedValueOnce({
            data: {
                ...
                formatStreams: [
                    { container: "webm", qualityLabel: "1080p", bitrate: 1_500_000, type: "video/webm", itag: 45 },
                    { container: "mp4", qualityLabel: "720p", bitrate: 2_200_000, type: "video/mp4", itag: 22 },
                    { container: "mp4", qualityLabel: "1080p", bitrate: 1_800_000, /* type intentionally missing */ itag: 22 },
                ],
            },
            headers: {},
        });
