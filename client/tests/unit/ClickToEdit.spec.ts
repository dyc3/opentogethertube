...
describe('ClickToEdit', () => {
	...
	it.concurrent.each([[5, "101"], ["five", "five"]])('renders only number values with formatter', (testVal, expected) => {
		const wrapper = shallowMount(ClickToEdit, {
			propsData: {
				value: testVal,
	...
	it.each([[5, 10], ["five", "ten"]])("should correctly implement v-model, and emit change when value applied", async (startVal, emitVal) => {
		const wrapper = mount({
			data() {
				return {
					ligma: startVal,
